...
public class LootEconHandler {
    ...
    public static void initialize(Server server) {
        ...
        if (iConomy5 != null) {
            ...
            log.info(CombatEventsLoot.plugName + " - Economy enabled using: iConomy v" + version);
        } else {
            handler = EconomyHandler.NONE;
            log.warning(CombatEventsLoot.plugName + " - An economy plugin isn't loaded.");
        }
    }
    ...
    public static void rewardPlayer(String player, double amount) {
        if (handler == EconomyHandler.ICONOMY5) {
            ...
            if (pAccount == null) {
                log.warning(CombatEventsLoot.plugName + " - Error fetching iConomy account for " + player);
            } else
    ...
    public static void setHandler(EconomyHandler handler) {
        LootEconHandler.handler = handler;
    }
